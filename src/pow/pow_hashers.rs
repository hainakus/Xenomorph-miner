...
impl PowHash {
    #[inline]
    pub fn new(pre_pow_hash: LHash, timestamp: u64) -> Self {
    ...
    #[inline(always)]
    pub fn finalize_with_nonce(&mut self, nonce: u64) -> LHash {
        self.0.update(&nonce.to_le_bytes());
        let mut hash_bytes = [0u8; 32];
        self.0.finalize_xof().fill(&mut hash_bytes);
        LHash::from_bytes(hash_bytes)
    }
}

impl KHeavyHash {
    #[inline]
    pub fn hash(in_hash: LHash) -> LHash {
...
mod tests {
    use crate::Hash;
    use crate::pow::lib::LHash as BHash;
    ...
    fn test_pow_hash() {
        ...
        let pre_pow_hash = BHash::from_bytes([
